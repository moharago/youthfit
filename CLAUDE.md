...
| DB | Supabase (PostgreSQL) — 현재 MySQL에서 마이그레이션 예정 |
...
├── database.py      # DB 연결 & 쿼리 (MySQL → Supabase 교체 예정)
├── user_service.py  # 사용자 정보 추출 & 저장
├── data/files/      # 원본 정책 문서 (CSV, PDF, Excel)
└── .env             # API 키 (절대 커밋 금지)

frontend/
└── src/App.jsx      # 챗봇 UI, API 호출
```
...
3. ⬜ DB: MySQL → Supabase 교체
4. ⬜ Frontend: package.json 복구 + Vite 세팅
